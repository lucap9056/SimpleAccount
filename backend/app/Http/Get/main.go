...
func Handler(author *AccountStruct.User, auth *Auths.Auth, db *Database.API, writer http.ResponseWriter, req *http.Request) {
	url := Url.New(req.URL)
	...
	switch url.Shift() {
	case "user":
		result, errCode, err = User(author, auth, writer, req)
	case "register":
		errCode, err = RegisterVerify(db, url.Shift())
	default:
